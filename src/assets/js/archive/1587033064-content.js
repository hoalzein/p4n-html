...
function runFunctionsByDataTag(dataTag, removeTagAfterProceeding)
{
...
function initRippleEvent() {
    ...
    if (rippleElementList.length > 0) {
        for (var element in rippleElementList) {
            if (rippleElementList.hasOwnProperty(element)) {
                initRippleEventForElement(rippleElementList[element]);
            }
        }
    }
}
...
function initRippleEventForElement(element) {
    $(element).unbind('click.rippleEffect').bind( "click.rippleEffect", function($event) {
...
function template_fileinput_required($that) {
    ...
    input.unbind("blur.required").bind("blur.required",function (e) {
        template_fileinput_required_($that);
    });
...
{
    $('.template_fileinput input[type="file"]').each(function() {
        console.log($(this));
        $(this).unbind('change.file_selected').bind('change.file_selected',function($event){
            let fileName = $event.target.files[0].name;
            setUploadFileName($event, fileName);
        });
    });

    $('.template_fileinput .file-reset').each(function() {
        $(this).unbind('click.file_reset').bind('click.file_reset',function($event){            
            $($event.target).closest('.template_fileinput').find('input[type="file"]').val('');
            setUploadFileName($event, '');
        });
    });
}


function setUploadFileName($event, fileName)
{
    let $fileInputWrapper = $($event.target).closest('.template_fileinput');

    if (fileName)
    {
        $fileInputWrapper.find('.custom-file-input .file-empty-text').removeClass('visible');
    }
    else {
        $fileInputWrapper.find('.custom-file-input .file-empty-text').addClass('visible');
    }

    $fileInputWrapper.find('.custom-file-input .file-name').text(fileName);
}
function allRequiredFields() {
    ...
    $("form .template_datuminput.required").each(function() {
        test=template_datuminput_required_($(this));
        if (test==false) {
            check=false;
        }
    });
    ...
    $("form .template_signature.required").each(function() {
        test=templateSignaturRequired_($(this));
        if (test==false) {
            check=false;
        }
    });
            ...
            if (form_ele.attr("method") == "GET" || form_ele.attr("method") == "get") {
                ...
                RequestHelper.get(url, function (response) {
                    ...
                    setTimeout(function() {
                        ...
                        if (target!="main") {
                            jq1112("div[name='" + target + "']").find("a").each(function() {
                                if (requestResponseToContent_checkLink($(this).attr("href"))) {
                                    if (typeof $that.attr("target") != typeof undefined && $that.attr("target") != "") {} else {
                            ...
                            jq1112("div[name='" + target + "']").find("form").each(function() {      
                                if (typeof $that.attr("target") != typeof undefined && $that.attr("target") != "") {} else {
                                    $(this).attr("target", target);
...
function requestResponseToContent_checkLink(url) {
    if (
...
function requestResponseToContent_delete($that) {
    $that.attr("target", $that.attr("data-target"));
    $that.attr("data-target", "");
    $that.attr("href", $that.attr("data-href"));
    $that.attr("data-href", "");
    $that.unbind("click.changeTarget");
}
...
function requestResponseToContent($that) {
    ...
    if ($that.hasClass("template_tab_link")) {
        $that.trigger("click");
    }
    }
...
var select_js_arr = new Array();
var select_js_arr_z = 0;


function select_js_calc(container, select_click, options, ontop) {
    var richtung = "";

    var select_click_height = select_click.outerHeight();
    var select_click_pos_top = parseInt(select_click.parent().offset().top);
    var select_click_pos_left = parseInt(select_click.offset().left);
    var doc_scroll_top = parseInt(jq1112(document).scrollTop());
    var li_height = options.find("li").not(".option-optgroup").eq(0).outerHeight();
    var window_height = parseInt(jq1112(window).height());


    var window_height = (jq1112(window).height() - jq1112("div[name='tabs']").height() - 10);


    var calc = select_click_pos_top + select_click_height;
    var mitte = (parseInt(jq1112(window).height()) / 2) + doc_scroll_top;

    var spezialfall = false;
    try {
        if (top && topmain && jq1112(".fancybox-inner", topmain.document).find(".fancybox-iframe").contents().find(".fancybox-inner").length > 0 && global_ie11) {
            spezialfall = true;
        }
    } catch (e) {
    }
    ...
    if (spezialfall) {
        container.css("position", "-ms-page");
    }
    ...
    if (calc <= mitte || ontop) {
        ...
        if (ontop) {
            ...
            container.css("top", ((select_click_pos_top - (spezialfall ? 0 : doc_scroll_top)) + select_click_height));
        }

        container.css("left", select_click_pos_left);


        var calc_height = window_height - (calc) + doc_scroll_top - li_height;

        if (ontop) {
            calc_height = window_height - (calc) + 0 - li_height;
        }

        var drei2 = calc_height / li_height;

        //  console.log(window_height+" "+select_click_pos_top+" "+calc_height+" "+li_height+" "+(Math.floor(drei2)*li_height));
        options.css("max-height", (Math.floor(drei2) * li_height) + "px").css({
            "overflow-y": "auto",
            "overflow-x": "hidden"
        });
        ...
        if (ontop) {
            if (!select_click.parent().next().hasClass("select-js-menudesign")) {
                var design = topmenu.jq1112("<div class='select-js-menudesign' style='position:absolute; display:none;'></div>'");
                //<input style='position:relative;z-index:0;' type='text' value='' /><div style='top:0px;position:absolute;display:block;z-index:1;width:100%;height:100%;background:transparent'></div>
                design.insertAfter(select_click.parent());
                design.css("background", "#fff");

                design.css("height", "13px");
                design.css("overflow", "hidden");
                design.css("left", (Math.floor(select_click.offset().left)));
                design.css("bottom", "0px");
                design.css("border-left", "1px solid #999");
                design.css("border-right", "1px solid #999");
                design.css("border-top", "1px solid #999");
                design.find("input").css("border-top", "0px");
                design.find("input").css("width", "100%");
                //design.find("input").css("margin-top","-5px");
                container.css("border-top", "0px");
                container.find("input").css("border-top", "0px");
            } else {
                design = select_click.parent().next(".select-js-menudesign");
            }
            design.css("width", container.outerWidth());
            design.css("display", "block");
        }

    } else {

        var eins = window_height + doc_scroll_top; //max (bildschirmrand)
        var zwei = eins - select_click_pos_top;
        var drei = window_height - zwei - (scrollbarWidth() * 2);
        var drei2 = drei / li_height;
        if (drei2 == 0) {
            drei2 == 25;
        }
        options.css("max-height", (Math.floor(drei2) * li_height) + "px").css({
        ...
        container.css("top", ((select_click_pos_top - (spezialfall ? 0 : doc_scroll_top)) - container.height() - 2));
        container.css("left", select_click_pos_left);
        richtung = "up";
    }


    if (parseInt(jq1112("html").scrollLeft()) > 0) {
        container.css("left", (select_click_pos_left - (spezialfall ? 0 : jq1112("html").scrollLeft())));
    } else {
        if ((select_click.offset().left + container.outerWidth() + 1) > jq1112(window).width()) {
            var calc = jq1112(window).width() - (select_click_pos_left + container.outerWidth() + 1);
            container.css("left", "+=" + calc);
            if ((select_click_pos_left + calc) < 5) {
                container.css("left", "5px");
                container.css("max-width", jq1112(window).width() - 10);
                container.addClass("select_container_big");
            }
            if (ontop) {
                design.css("left", container.offset().left);
            }
        }
    }
...
function findNext(ele) {
    if (ele.length > 0 && ele.next().length > 0 && ele.next()[0].tagName.toLowerCase() == "li") {
        if (ele.next().hasClass("sichtbar")) {
            findNext_ele = ele.next();
        } else {
            findNext(ele.next());
        }
    }
...
function findPrev(ele) {
    if (ele.length > 0 && ele.prev().length > 0 && ele.prev()[0].tagName.toLowerCase() == "li") {
        if (ele.prev().hasClass("sichtbar")) {
            findPrev_ele = ele.prev();
        } else {
            findPrev(ele.prev());
        }
    }
...
findPrev_optgroup_ele = null;

function findPrev_optgroup(ele) {
    //return ele.prevUntil(".option-optgroup");
    if (ele.length > 0 && ele.prev().length > 0 && ele.prev()[0].tagName.toLowerCase() == "li") {
        if (ele.prev().hasClass("option-optgroup")) {
            findPrev_optgroup_ele = ele.prev();
        } else {
            findPrev_optgroup(ele.prev());
        }
    }
    return null;
}

function select_js_focus(ele) {
    jq1112(ele).unbind("keypress").bind("keypress", function (e) {
        if (e.which == 13) {
            jq1112(this).parent().find(".select-js-click").trigger("click");
            e.preventDefault();
        }
    });
}

function select_js_dispatch() {
    try {
        jq1112(".select-js-click", topmenu.document).unbind();
        jq1112(".select_open", topmenu.document).removeClass("select_open");
        jq1112(".select-js-event-ready", topmenu.document).removeClass("select-js-event-ready");
    } catch (e) {
    }
    try {
        if (top && topmenu && topmenu.document) {
            jq1112("html", topmenu.document).unbind('click.select-js-outside');
        }
    } catch (e) {
    }
    try {
        if (top && topmain && topmain.document) {
            jq1112("html", topmain.document).unbind('click.select-js-outside');
        }
    } catch (e) {
    }

    try {
        jq1112(".select-js-click", topmain.gmekonfig_menu.document).unbind();
        jq1112(".select_open", topmain.gmekonfig_menu.document).removeClass("select_open");
        jq1112(".select-js-event-ready", topmain.gmekonfig_menu.document).removeClass("select-js-event-ready");
    } catch (e) {
    }
    try {
        jq1112("html", topmain.gmekonfig_menu.document).unbind('click.select-js-outside');
    } catch (e) {
    }
    try {
        jq1112("html", topmain.gmekonfig_main.document).unbind('click.select-js-outside');
    } catch (e) {
    }

    jq1112("html").unbind('click.select-js-outside');
    clearInterval(select_js_timers);
    select_js_timers2 = null;
    select_js_timers = null;

    //jq1112("html",topmain.document).unbind('keypress.select-js');


    // console.log("DISPATCH");
    jq1112(".select-js-event-ready", document).removeClass("select-js-event-ready");


    /*jq1112(".frameclass", top.document).each(function() {
        var frame=jq1112(jq1112(this, top.document)[0].contentDocument);
        frame.find(".select-js").removeClass("select-js-event-ready");
        frame.find(".select-js-click").unbind();
        frame.find(".select_container").unbind();
        frame.find(".select_container").find("input").unbind();
        frame.find(".select_options").find("li").unbind();

        clearInterval(frame.select_js_timers);
        frame.select_js_timers=null;
        frame.select_js_timers2=0;
        frame.select_js_options=[];
    });*/
}

function select_js_dispatch_all() {
    try {
        jq1112(".select-js").each(function () {
            var container = $("body").find(".select_container");


            var select_click = jq1112(this).parent().find(".select-js-click");
            select_js_close(container, select_click, false);
        });
    } catch (e) {
    }
    try {
        jq1112(".select-js", topmenu.document).each(function () {

            var container = $("body").find(".select_container");

            var select_click = jq1112(this).parent().find(".select-js-click");
            select_js_close(container, select_click, true);
        });
    } catch (e) {
    }

    try {
        jq1112(".select-js", topmain.gmekonfig_menu.document).each(function () {
...
function select_js_all() {
    jq1112(".select-js-all").each(function () {
        jq1112(this).removeClass("select-js-all");
        jq1112(this).attr("onfocus", "if (typeof select_js_focus == \'function\') {select_js_focus(this);}");
        select_js_element(jq1112(this)[0]);
    });
}

function select_js_element(ele2) {
    select_js_dispatch_all();
    var ontop = false;
    var doc = ele2.ownerDocument;
    if (topmenu !== undefined && doc == topmenu.document) {
        ontop = true;
    }
    ...
    var select = jq1112(ele2, doc).parent().find(".select-js");


    //jq1112(".select-js",doc.document).each(function() {

    //var select=jq1112(this);

    var temp_id = "";
    if (ontop) {
        temp_id = "select-js-id-" + select.attr("name");
    }

    if (!select.hasClass("select-js-ready")) {

        select.addClass("select-js-ready");

        select.parent().append(jq1112('<span class="select-js-click" onclick="if (this.previousSibling && this.previousSibling.hasAttribute && this.previousSibling.hasAttribute(\'disabled\')) {return false;} if (this.className.indexOf(\'select_remove\')===-1) { if (typeof select_js_element==\'function\' && this.className.indexOf(\'select-js-event-ready\')===-1) { select_js_element(this); } } else { this.className=this.className.replace(\' select_remove\',\'\'); }"></span>'));

        /*jq1112("option",select).each(function() {
            options+="<li "+(jq1112(this).attr("value")==select.val()?"class='option-selected'":"")+" data-key='"+jq1112(this).attr("value")+"' data-val='"+jq1112(this).text()+"'>"+jq1112(this).text()+"</li>";
        });
        jq1112("option:not(:selected)", select).remove();


        var html="<div class='"+temp_id+" select_container' style='display:none;'><div class='select_suchfeld'><input name='' value='' type='text' /></div><ul class='select_options'>"+options+"</ul><div class='select_suchfeld2'><input name='' value='' type='text' /></div></div>";


        jq1112(html).insertAfter(select.parent());*/

    } else {
        var options = "";
        var last_optgroup = "";
        var mit_icons = false;
        jq1112("option,optgroup", select).each(function () {

            var attrs = "";
            var className = "sichtbar";

            jq1112.each(jq1112(this)[0].attributes, function (index, attribute) {
                if (attribute.name != "class" && attribute.name != "data-key" && attribute.name != "data-val" && attribute.name != "value") {
            ...
            if (jq1112(this)[0].nodeName.toLowerCase() == "optgroup") {
                last_optgroup = " (" + escapeHtml(jq1112(this).attr("label")) + ")";
                options += "<li class='option-optgroup'>" + escapeHtml(jq1112(this).attr("label")) + "</li>";
            } else {
                var prev_icon = jq1112(this).attr("data-prev-icon");
                if (typeof prev_icon != typeof undefined && prev_icon != "" && prev_icon != null) {
                    prev_icon = "<i class='material-icons'>" + prev_icon + "</i>";
                    mit_icons = true;
                } else {
                    prev_icon = "";
                }

                options += "<li " + attrs + " class='" + className + (jq1112(this).attr("value") == select.val() ? " option-selected" : "") + "' data-key='" + escapeHtml(jq1112(this).attr("value")) + "' data-opt='" + last_optgroup + "' data-val='" + escapeHtml(jq1112(this).text()) + "'>" + prev_icon + " " + escapeHtml(jq1112(this).text()) + "</li>";
        ...
        var html = "<div class='" + temp_id + " select_container" + multipleClass + "' style='display:none;'>\n\
                        <div class='select_suchfeld " + (mit_icons ? 'nicht_sichtbar' : '') + "'>\n\
                            <input name='select_suchfeld_" + Date.now() + Math.random() + "' value='' type='text' autocomplete='off' placeholder='" + lang_suche + "'/>\n\
                        </div>\n\
                        <ul class='select_options'>" + options + "</ul>\n\
                        <div class='select_suchfeld2'>\n\
                            <input name='select_suchfeld_" + Date.now() + Math.random() + "' value='' type='text' autocomplete='off' />\n\
                        </div>\n\
                    </div>";
        ...
        if (ontop) {
            if (jq1112("." + temp_id).length <= 0)
                jq1112("body").append(select.parent().next(".select_container").clone());
        }

        if (!select.hasClass("select-js-event-ready") || ontop) {
            select.addClass("select-js-event-ready");
            select.parent().find(".select-js-click").addClass("select-js-event-ready");
            if (ontop) {
                select_js(select.next()[0], doc, temp_id);
            } else {
                select_js(select.next()[0], doc);
            }
        }
    }
    //});
...
function getAttributes($node) {
    var attrs = {};
    jq1112.each($node[0].attributes, function (index, attribute) {
        attrs[attribute.name] = attribute.value;
    });
...
var select_js_timers;
var select_js_timers2;
var select_js_options;


function select_js_key_timer(that, c) {
    var searchValue1 = that.val();
    //setTimeout(
    // function(){
    if (
        (searchValue1 == that.val() && searchValue1 != null)
    ) {
...
function select_js_close(container, select_click, ontop) {
    select_js_options = [];
    clearInterval(select_js_timers);
    select_js_timers = null;
    select_js_timers2 = null;
    ...
    //setTimeout( function() {
    if (ontop) {
        select_click.parent().next(".select-js-menudesign").remove();
    }
    container.css({"display": "none", "width": ""});
    container.find(".select_options").css("max-height", "");
    container.remove();
    select_click.removeClass("select_open");
    select_js_dispatch();
    //},200);
}


function select_js(ele, doc, temp_id) {
    /*var doc=topmain;
    var ontop=false;
    if (typeof doc2 != typeof undefined) {
        doc=doc2;
        if (doc==topmenu) {
            ontop=true;
        }
    ...
    var select = jq1112(ele, doc).parent().find("select");
    //select.css({"position":"relative","z-index":"0"});


    if (typeof temp_id != typeof undefined && temp_id != "") {
        var container = jq1112("." + temp_id);
    } else {
        var container =/*doc.*/jq1112("body").find(".select_container");

    }


    var select_click = jq1112(ele, doc);
    //  select_click.removeClass("select_open");


    select_click.unbind("click").bind("click", function () {
        clearInterval(select_js_timers);
        select_js_timers2 = null;
        select_js_timers3 = null;
        select_js_timers = null;

        var options = container.find(".select_options");
        var li = options.find("li");
        var suchfeld = container.find(".select_suchfeld");
        var suchfeld2 = container.find(".select_suchfeld2");

        if (li.length > 1 && !suchfeld.hasClass("nicht_sichtbar")) {
            suchfeld.css("display", "block");
        } else {
            suchfeld.css("display", "none");
        }
        suchfeld2.css("display", "none");
        //  container.removeClass("durchgang");
        //container.css("display","block").css("width","auto");
        //container.attr("data-width",container.outerWidth()+scrollbarWidth());
        //container.attr("data-height",container.outerHeight());


        container.css("display", "block");
        container.css("width", container.outerWidth() + scrollbarWidth());

        var richtung = select_js_calc(container, select, options, ontop, true);
        ...
        /* container.css("width","").addClass("transstart");
         jq1112(".select-js-menudesign",topmenu.document).css("width","0px");
        
            setTimeout(function() {
                container.addClass("transstart");
                container.css({"width":container.attr("data-width")+"px"});
            
                
                jq1112(".select-js-menudesign",topmenu.document).css("width",container.attr("data-width")+"px").addClass("transstart");
            },1);*/

        var suchefeld = suchfeld.find("input");


        options.find(".option-selected").removeClass("option-selected");
        /*var selected_option = select.find("option:selected");
        if (selected_option.length<=0) {
            selected_option=li.eq(0);
        }*/
        //console.log(select.val()+" "+escapeHtml(select.val()));
        //console.log(options.find("li"));

        var gef = false;
        options.find("li").each(function () {
            var a = jq1112(this).attr("data-key");
            //console.log(select.val()+" "+a+" "+a.indexOf(select.val()));
            if (a && select.val() != "" && a == select.val()) {
                jq1112(this).addClass("option-selected");
                gef = true;
            }
        });

        if (gef) {
            //options.find("li[data-key=\""+(select.val())+"\"]").addClass("option-selected");
        } else {
            li.eq(0).addClass("option-selected");
            if (li.eq(0).hasClass("option-optgroup")) {
                findNext_ele = null;
                findNext(li.eq(0));
                li.eq(0).removeClass("option-selected");
                findNext_ele.addClass("option-selected");
            }
        }

        if (select_click.hasClass("select_open")) {
            select_click.addClass("select_remove");


            select_js_close(container, select_click, ontop);
            ...
            li.each(function () {
                ...
                jq1112(this).removeClass("durchgang").addClass("sichtbar");
                //jq1112(this).css("display","");

                if (typeof select.attr('multiple') === 'undefined' || jq1112(this).attr('data-key') === '-1') {
                    // single select + multiple[data-key="-1"] ...
                    jq1112(this).unbind("click").bind("click", function () {
                        if (!jq1112(this).hasClass("option-optgroup")) {
                            li.removeClass("option-selected");
                            jq1112(this).addClass("option-selected");
                            var icon = jq1112(this).attr("data-prev-icon");
                            if (typeof icon != typeof undefined && icon != "" && icon != null) {
                                var res = icon.split(" ");
                                select.closest(".styled-select").find("i").text(res[0]);
                            }
                            select.val(jq1112(this).attr("data-key"));
                            select.closest(".styled-select").find('.custom-select-value .text').text(jq1112(this).attr("data-val"));
                            select_update_text(select);
                            select_js_close(container, select_click, ontop);
                            select.focus();
                            ...
                            if (typeof select.attr('multiple') !== 'undefined' && jq1112(this).attr('data-key') === '-1')
                            {
                                ...
                                jq1112(this).removeAttr('selected');
                                select_update_text(select);
                            }
                        }
                    });
                ...
                if (typeof select.attr('multiple') !== 'undefined') {
                    // multiple select
                    if (jq1112(this).attr('data-key') !== '-1')
                    {
                        jq1112(this).unbind("click").bind("click", function () {
                            if (!jq1112(this).hasClass("option-optgroup")) {
                                li.removeClass("option-selected");
                                ...
                                if (typeof jq1112(this).attr('selected') === 'undefined')
                                {
                                    select.find('option[value="'+jq1112(this).attr("data-key")+'"]').attr('selected','selected');
                                    jq1112(this).attr('selected','selected');
                                }
                                else {
                                    select.find('option[value="'+jq1112(this).attr("data-key")+'"]').removeAttr('selected');
                                    jq1112(this).removeAttr('selected');
                                }
                            }
                            select_update_text(select);
                ...
                if (!jq1112(this).hasClass("option-optgroup")) {
                    ...
                    var text = jq1112(this).html();
                    var dataval = jq1112(this).attr("data-val");
                    var dataopt = jq1112(this).attr("data-opt");

                    if (text != dataval && !jq1112(this).attr("data-prev-icon")) {
                        jq1112(this).html(dataval);
                        text = dataval;
                    }

                    var temp = {};
                    temp.text = text;
                    temp.ele = jq1112(this);
                    temp.data_val = dataval;
                    temp.data_opt = dataopt;

                    //findPrev_optgroup_ele=null;


                    if (options.find(".option-optgroup").length > 0) {
                        if (check) {
                            //console.log("hm");
                            findPrev_optgroup(jq1112(this));
                            check = false;
                        }
                    }

                    temp.optgroup = findPrev_optgroup_ele;

                    select_js_options.push(temp);
                } else {
                    check = true;
                }
            });
            ...
            if (!ontop) {
                select_js_timers2 = (select_click.offset().top - jq1112(document).scrollTop());
                select_js_timers3 = (select_click.offset().left - jq1112(document).scrollLeft());
                select_js_timers = setInterval(function () {
                    if ((select_click.offset().top - jq1112(document).scrollTop()) != select_js_timers2) {
                        select_js_close(container, select_click, ontop);
                    }
                    if ((select_click.offset().left - jq1112(document).scrollLeft()) != select_js_timers3) {
                        select_js_close(container, select_click, ontop);
                    }
                }, 200);
            }
            ...
            suchefeld.unbind("keypress").bind("keypress", function (e) {

                if (e.which == 13) {
                    selected_option = options.find(".option-selected");
                    if (selected_option.length <= 0) {
                        selected_option = options.find("li").eq(0);
                        li.removeClass("option-selected");
                        selected_option.addClass("option-selected");
                    }
                    if (selected_option.length > 0) {
            ...
            var selected_option_height = 0;
            var selected_option_pos_top = 0;
            var options_height = 0;

            suchefeld.unbind("keydown").bind("keydown", function (event) {
                //if (typeof temp_id != typeof undefined && temp_id!="") {
                //container=topmain.jq1112("."+temp_id);
                //} else {
                //container=jq1112(ele).parent().next(".select_container");
                //}
                ...
                if (event.which == 40) {
                    ...
                    if (selected_option.length <= 0) {
                        selected_option = container.find(".sichtbar").eq(0);
                        li.removeClass("option-selected");
                        selected_option.addClass("option-selected");
                    }

                    findNext_ele = null;
                    findNext(selected_option);

                    if (findNext_ele != null && selected_option.length > 0 && findNext_ele.length > 0) {
                        li.removeClass("option-selected");
                        findNext_ele.addClass("option-selected");


                        selected_option = findNext_ele;
                        selected_option_height = parseInt(selected_option.outerHeight());
                        selected_option_pos_top = parseInt(selected_option.position().top);
                        options_height = parseInt(options.outerHeight()) - options.position().top;


                        if (
                            (richtung == "down" && ((selected_option_pos_top - (selected_option_height * 2)) > options_height)) ||
                            (richtung == "up" && ((selected_option_pos_top + selected_option_height) > options_height))
                        ) {
                            options.scrollTop(parseInt(options.scrollTop()) + selected_option_height);
                        }
                        if (findNext_ele != null && findNext_ele.hasClass("option-optgroup")) {
                            var e = jq1112.Event("keydown");
                ...
                if (event.which == 38) {
                    li.unbind('mouseenter');
                    if (selected_option.length <= 0) {
                        selected_option = container.find(".sichtbar").last();
                    }
                    /*if (!selected_option.hasClass("sichtbar")) {
                        selected_option=container.find(".sichtbar").last();
                        if (richtung=="up") {
                            selected_option.parent().scrollTo(container.find(".sichtbar").last());
                        }
                    }*/


                    findPrev_ele = null;
                    findPrev(selected_option);

                    if (findPrev_ele != null && selected_option.length > 0 && findPrev_ele.length > 0) {
                        li.removeClass("option-selected");
                        findPrev_ele.addClass("option-selected");

                        selected_option = findPrev_ele;
                        selected_option_height = parseInt(selected_option.outerHeight());
                        selected_option_pos_top = parseInt(selected_option.position().top);
                        options_height = parseInt(options.height());

                        if (
                            (richtung == "down" && (selected_option_pos_top < selected_option_height)) ||
                            (richtung == "up" && (selected_option_pos_top < 0))
                        ) {
                            options.scrollTop(parseInt(options.scrollTop()) - selected_option_height);
                        }
                    }
                    if (findPrev_ele != null && findPrev_ele.hasClass("option-optgroup")) {
                        var e = jq1112.Event("keydown");
            ...
            container.unbind("mousemove").bind("mousemove", function () {
            ...
            /*jq1112("html",topmain.document).bind("keypress.select-js", function (e) {
            ...
            /* suchefeld.unbind("blur").bind("blur",function(e) {
                 console.log("blur");
                 //select_js_close(container,select_click,ontop);
             });*/

            jq1112("html").bind('click.select-js-outside', function (e) {
                if (
                    (select.parent().find(e.target).length === 0) &&
                    (jq1112(e.target).closest(".select_container").length === 0)
                ) {
                    select_js_close(container, select_click, ontop);
                }
            });
            if (top && topmain && topmain.document) {
                try {
                    jq1112("html", topmain.document).bind('click.select-js-outside', function (e) {
                        if (
                            (select.parent().find(e.target).length === 0) &&
                            (jq1112(e.target).closest(".select_container").length === 0)
                        ) {
            ...
            if (top && topmenu && topmenu.document) {
                jq1112("html", topmenu.document).bind('click.select-js-outside', function (e) {
                    if (
                        (select.parent().find(e.target).length === 0) &&
                        (jq1112(e.target, topmain.document).closest(".select_container").length === 0)
                    ) {
                        select_js_close(container, select_click, ontop);
                    }
                });
            ...
            try {
                jq1112("html", topmain.gmekonfig_main.document).bind('click.select-js-outside', function (e) {
                    if (
                        (select.parent().find(e.target).length === 0) &&
                        (jq1112(e.target, topmain.gmekonfig_main.document).closest(".select_container").length === 0)
                    ) {
                        select_js_close(container, select_click, ontop);
                    }
                });
            ...
            try {
                jq1112("html", topmain.gmekonfig_menu.document).bind('click.select-js-outside', function (e) {
                    if (
                        (select.parent().find(e.target).length === 0) &&
                        (jq1112(e.target, topmain.gmekonfig_menu.document).closest(".select_container").length === 0)
                    ) {
                        select_js_close(container, select_click, ontop);
                    }
                });
            ...
            suchefeld.unbind("keyup").bind("keyup", function (event) {
                //if (typeof temp_id != typeof undefined && temp_id!="") {
                //container=topmain.jq1112("."+temp_id);
                //} else {
                //container=jq1112(ele).parent().next(".select_container");
                ...
                if (event.which == 40 || event.which == 38) {
                    ...
                    // select_js_key_timer(jq1112(this), function( that ) {

                    var that = jq1112(this);
                    var searchtext = that.val();

                    if (ontop) {
                        select_click.parent().next(".select-js-menudesign").find("input").val(searchtext);
                    }

                    li.removeClass("durchgang");
                    for (var i = 0; i < select_js_options.length; i++) {
                        if (select_js_options[i].text != select_js_options[i].data_val) {
                            select_js_options[i].ele.html(select_js_options[i].data_val);
                            select_js_options[i].text = select_js_options[i].data_val;
                        }
                        ...
                        var dataval = select_js_options[i].data_val;
                        var dataopt = select_js_options[i].data_opt;
                        var results = dataval.match(regex);
                        var results2 = dataval.match(regex2);
                        var results3 = dataval.match(regex3);
                        ...
                        if ((results != null && results.length > 0 && results[0] != "") || (results2 != null && results2.length > 0 && results2[0] != "") || (results3 != null && results3.length > 0 && results3[0] != "")) {
                            select_js_options[i].ele.addClass("sichtbar");

                            if (results != null && results.length > 0 && results[0] != "") {
                                var result2 = dataval.replace(results[0], "<b><u>" + results[0] + "</u></b>");
                            }

                            if (results2 != null && results2.length > 0 && results2[0] != "") {
                                var result2 = dataval.replace(results2[0], "<b><u>" + results2[0] + "</u></b>");
                            }

                            if (results3 != null && results3.length > 0 && results3[0] != "") {
                                var result2 = dataval.replace(results3[0], "<b><u>" + results3[0] + "</u></b>");
                            }

                            select_js_options[i].ele.html(result2);//+dataopt 
                            select_js_options[i].text = result2;//+dataopt;


                            if (select_js_options[i].optgroup != null && !select_js_options[i].optgroup.hasClass("durchgang")) {
                                select_js_options[i].optgroup.addClass("sichtbar").addClass("durchgang");
                            }
                        } else {
                            select_js_options[i].ele.removeClass("sichtbar");

                            if (select_js_options[i].optgroup != null && !select_js_options[i].optgroup.hasClass("durchgang")) {
                                select_js_options[i].optgroup.removeClass("sichtbar");
                            }
                        }
                    }

                    if (searchtext == "") {
                        li.addClass("sichtbar");
                    }


                    select_js_calc(container, select, options, ontop, false);

                    //li.unbind('mouseenter');

                    selected_option = options.find(".option-selected");
                    if (selected_option.length <= 0 || !selected_option.hasClass("sichtbar")) {
                        selected_option = options.find(".sichtbar").eq(0);
                        li.removeClass("option-selected");
                        selected_option.addClass("option-selected");
                        if (selected_option.hasClass("option-optgroup")) {
                            findNext_ele = null;
                            findNext(selected_option);
                            selected_option.removeClass("option-selected");
                            findNext_ele.addClass("option-selected");
                        }
                    }

                    options.scrollTo(selected_option);
                    // });
                    /*li.unbind('mouseenter').mouseenter(function() {
                        if (!jq1112(this).hasClass("option-optgroup")) {
                            li.removeClass("option-selected");
            ...
            var selected_option = options.find(".option-selected");
            if (selected_option.length <= 0 || !selected_option.hasClass("sichtbar")) {
                selected_option = options.find(".sichtbar").eq(0);
                li.removeClass("option-selected");
                selected_option.addClass("option-selected");
            ...
            select_click.addClass("select_open");
            // container.css("display","block");
            suchefeld.focus();
            // suchfeld.trigger("keyup");
        }
    });
...
findPrev_optgroup_ele_multi = null;

function findPrev_optgroup_multi(ele) {
    if (ele.length > 0 && ele.prev().length > 0 && (ele.prev()[0].tagName.toLowerCase() == "li")) {
        if (ele.prev().hasClass("option-optgroup")) {
            findPrev_optgroup_ele_multi = ele.prev();
        } else {
            findPrev_optgroup_multi(ele.prev());
        }
    }
    return null;
}
...
function select_js_multi() {
    if (jq1112(".select_container_multi").length > 0) {
        jq1112(".select_container_multi").each(function () {

            var select_suchfeld = jq1112(this).find(".select_suchfeld").find("input");
            var select = jq1112(".select_container_multi").find("ul");
            var options = select.find("li").not(".option-optgroup");

            select_suchfeld.unbind("keyup").bind("keyup", function (event) {
                if (event.which == 40 || event.which == 38) {
                } else {

                    select.find("li").removeClass("sichtbar");
                    var that = jq1112(this);
                    var searchtext = that.val();
                    for (var i = 0; i < options.length; i++) {
                        ...
                        var dataval = jq1112(options[i]).text();
                        //  console.log(dataval);
                        ...
                        if ((results != null && results.length > 0 && results[0] != "") || (results2 != null && results2.length > 0 && results2[0] != "") || (results3 != null && results3.length > 0 && results3[0] != "")) {
                            ...
                            jq1112(options[i]).addClass("sichtbar");
                            ...
                            jq1112(options[i]).html(result2);//+dataopt 
                            jq1112(options[i]).text = result2;//+dataopt;

                            if (select.find(".option-optgroup").length > 0) {
                                findPrev_optgroup_multi(jq1112(options[i]));
                                if (findPrev_optgroup_ele_multi && findPrev_optgroup_ele_multi != null) {
                                    findPrev_optgroup_ele_multi.addClass("sichtbar");
                                }
                            }

                        } else {
                            jq1112(options[i]).removeClass("sichtbar");
                        }
                    }

                    if (searchtext == "") {
                        select.find("li").addClass("sichtbar");
                        select.find("li").each(function () {
                            jq1112(this).html(jq1112(this).html().split("<b><u>").join("").split("</u></b>").join(""));
                        });
                    }
...
var select_suche_multi_strg_z = 0;
var select_suche_multi_strg = false;

function select_suche_multi_init() {
    try {
        jq1112(document).unbind("keydown.strgz_select_suche_multi").bind('keydown.strgz_select_suche_multi', function (e) {
            if (e.keyCode == 17) {
                select_suche_multi_strg = true;
            }
        });
        jq1112(document).unbind("keyup.strgz_select_suche_multi").bind('keyup.strgz_select_suche_multi', function (e) {
            if (e.keyCode == 17) {
                select_suche_multi_strg = false;
            }
        });
    } catch (e) {
    }

    if (jq1112(".select_suche_multi") && jq1112(".select_suche_multi").length > 0) {
        jq1112(".select_suche_multi").not(".select_suche_multi_check").each(function () {
            var select = jq1112(this);
            select_suche_multi_strg_z++;
            select.attr("data-target", "select_container_multi_" + select_suche_multi_strg_z);


            var html = '<div class="select_container select_container_multi" id="select_container_multi_' + select_suche_multi_strg_z + '" style="display: block;">\n\
            <div class="select_suchfeld " style="display: block;">\n\
            <input name="" value="" type="text" autocomplete="off" placeholder="Suche">\n\
            </div>\n\
            <ul class="select_options" style="max-height: 375px; overflow-y: auto; overflow-x:hidden">\n\
            </ul>\n\
            </div>';
            //select.wrap("<div class='select_container_multi_wrap'></div>");
            if (select.closest("table").length > 0) {
                jq1112(html).insertAfter(select.closest("table"));
            } else {
                jq1112(html).insertAfter(select);
            }

            select.addClass("select_suche_multi_check").css("visibility", "hidden");
            var options = select.find("option,optgroup");
            var select_container = jq1112("#" + select.attr("data-target"));

            var select_suchfeld = select_container.find(".select_suchfeld").find("input");

            select_container.find("ul").html("");

            select_suche_multi_resize();

            for (var i = 0; i < options.length; i++) {
                var value = jq1112(options[i]).text();
                var optgroup = ""
                var class_ = "";
                if (jq1112(options[i]).attr("label") && jq1112(options[i]).attr("label") != "") {
                    value = jq1112(options[i]).attr("label");
                    optgroup = "option-optgroup";
                }
                if (jq1112(options[i]).attr("selected")) {
                    class_ = "option-selected";
                }
                select_container.find("ul").append('<li class="sichtbar ' + optgroup + ' ' + class_ + '" data-key="' + jq1112(options[i]).val() + '" data-val="' + jq1112(options[i]).text() + '">' + value + '</li>')
            }


            select_container.find("li").not(".option-optgroup").unbind("click").bind("click", function () {
                if (!select_suche_multi_strg) {
                    select.find("option").attr('selected', false).removeAttr("selected");
                    select_container.find("li").removeClass("option-selected");
                }

                if (jq1112(this).hasClass("option-selected")) {
                    jq1112(this).removeClass("option-selected");
                    select.find("option,optgroup").eq(jq1112(this).index()).attr('selected', false).removeAttr("selected");
                } else {
                    jq1112(this).addClass("option-selected");
                    select.find("option,optgroup").eq(jq1112(this).index()).attr('selected', true);
                }
                select.trigger("onchange");
                // setTimeout(function() { jq1112(select_suchfeld)[0].focus(); }, 10);
                try {
...
function select_suche_multi_resize() {

    jq1112(".select_suche_multi").each(function () {
        var select = jq1112(this);
        select.css("height", "auto");
        var select_container = jq1112("#" + select.attr("data-target"));
        var select_h = select.height();
        var select_w = select.width();

        select.css("height", select_h + 29);
        select_container.find("ul").css("width", select_w + "px");
        select_container.find("ul").css("height", select_h + "px");


        select_container.css("top", select.position().top + "px");
        select_container.css("left", select.position().left + "px");

        //  select_container.css("left",(select.position().left+200)+"px");
    });
}


function select_update_text(select) {
    var text = '';
    var i = 0;

    select.find('option').each(function () {
        if (jq1112(this).prop('selected') === true) {
            if (i > 0) {
                text += ', ';
            }
            text += ($(this).text());
    ...
    select.closest(".styled-select").find('.custom-select-value .text').text(text);
}/* 
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
...
function isInUpperHalf(ui, $droppable)
{
    ...
    if ((ui.offset.top + ($draggable.outerHeight() / 2)) < ($droppable.offset().top + ($droppable.outerHeight() / 3))) {
        rret= "above";
    }

    if (((ui.offset.top + ($draggable.outerHeight() / 2)) >= ($droppable.offset().top + ($droppable.outerHeight() / 3))) &&
            ((ui.offset.top + ($draggable.outerHeight() / 2)) <= ($droppable.offset().top + ($droppable.outerHeight() - ($droppable.outerHeight() / 3)))))

    {
       rret= "in";
    }

    if ((ui.offset.top + ($draggable.outerHeight() / 2)) > ($droppable.offset().top + ($droppable.outerHeight() - ($droppable.outerHeight() / 3)))) {
        rret=  "below";
    }
    return rret;
...
function updateHighlight(ui, $droppable)
{
    ...
    var a = isInUpperHalf(ui, $droppable);
    if (a == "above") {
        $droppable.removeClass("droppable-below").removeClass("droppable-in")
                .addClass("droppable-above");
    }
    if (a == "in") {
        $droppable.removeClass("droppable-below").removeClass("droppable-above")
                .addClass("droppable-in");
    }
    if (a == "below") {
        $droppable.removeClass("droppable-in").removeClass("droppable-above")
...
function cleanupHighlight(ui, $droppable)
{
    ...
    $droppable.removeClass("droppable-above droppable-below droppable-in");
}

function tableTreeView($that) {

    var table = $that.find("table");


    $("tr:not(.notsortaable)", table).find(".sortable_move").unbind("mousedown.draggable").bind("mousedown.draggable", function () {
        try {
            $that.find(".dtable > tbody").sortable("destroy");
        } catch (e) {
   
        }
        //  $("tr:not(.notsortaable):not([data-tree-ebene='"+0+"'])").addClass("canselsortable");
        //
        // $( "<tr class='sorthelper'><td colspan='100' style='background:rgba(82, 134, 186,.2);border-bottom:4px solid #fff'><div>dfsdsfdsf</div></td></tr>" ).insertAfter( $("tr[data-tree-ebene='"+0+"']:not(.notsortaable)") );
        //  $( "<tr class='sorthelper'><td colspan='100' style='background:rgba(82, 134, 186,.2);border-bottom:4px solid #fff' ><div  >dfsdsfdsf</div></td></tr>" ).insertBefore( $("tr[data-tree-ebene='"+0+"']:not(.notsortaable)") );
        //   $that.find(".dtable > tbody").append("<tr class='sorthelper'><td colspan='100' style='background:rgba(82, 134, 186,.2);border-bottom:4px solid #fff'><div  >dfsdsfdsf</div></td></tr>");

        var currentebene = $(this).closest("tr").attr("data-tree-ebene");

        if (typeof currentebene != typeof undefined) {
            var $next = $(this).closest("tr").next();
            var nextebene = parseInt($next.attr("data-tree-ebene"));

            while (nextebene > currentebene) {
                $next.css("opacity", "0.35").addClass("treeViweHelper");
                $next = $next.next();

                nextebene = parseInt($next.attr("data-tree-ebene"));
            }
        }


        $("tr:not(.notsortaable)", table).addClass("dragit");

        $("tr:not(.notsortaable)", table).draggable({
            ...
            drag: function (event, ui) {
                ...
                $(".treeViweHelper").css("top", ui.position.top + "px");

                var windowHeight = $(window).height();
                var mouseYPosition = event.clientY;
                var mouseYPosition = (ui.helper.offset().top);

                if ((mouseYPosition - jq1112(document).scrollTop()) < SCROLL_AREA_TOP_HEIGHT) {
                    currentlyScrolling = true;

                    $('html, body').animate({
                        scrollTop: "-=" + (ui.helper.height() * 3) + "px" // Scroll up half of window height.
                    },
                            400, // 400ms animation.
                            function () {
                ...
                } else if ((mouseYPosition + ui.helper.height() - jq1112(document).scrollTop()) > (windowHeight - SCROLL_AREA_BOTTOM_HEIGHT)) {

                    currentlyScrolling = true;

                    $('html, body').animate({
                        scrollTop: "+=" + (ui.helper.height() * 3) + "px" // Scroll down half of window height.
                    },
                            400, // 400ms animation.
                            function () {
                                currentlyScrolling = false;
            ...
            stop: function (event, ui) {
                $(".droppable-below").removeClass("droppable-below");
                $(".droppable-above").removeClass("droppable-above");
                $(".droppable-in").removeClass("droppable-in");
                $(".treeViweHelper").css("opacity", "1").css("top", "").removeClass("treeViweHelper");
                ui.helper.css("top","");
            }
        });


        $("tr:not(.notsortaable):not(.treeViweHelper)", table).droppable({
            accept: ".dragit",
            drop: function (event, ui) {

                var $this = $(this);
                var insert = isInUpperHalf(ui, $this);


                var direction = '';
                if (parseInt(ui.helper.attr('data-row-zaehler')) > parseInt($this.attr('data-row-zaehler'))) {
                    // richtung = "&unten=" + ui.helper.attr('data-row-id');
                    // target = $this.attr('data-row-id');
                    direction = "oben";
                } else {
                ...
                //alert(direction + " " + ui.helper.attr('data-row-zaehler') + " " + $this.attr('data-row-zaehler'));

                var helper = ui.helper.attr('data-row-id');
                ...
                RequestHelper.get(url, function (response) {
                    ...
                    var target = "main";
                    if (typeof $that.closest("form").attr("target") != typeof undefined && $that.closest("form").attr("target") != "") {
                        target = $that.closest("form").attr("target");
                    }
    ...
    $("tr:not(.notsortaable)", table).find(".sortable_move").unbind("mouseup.draggable").bind("mouseup.draggable", function () {
        $(".droppable-below").removeClass("droppable-below");
        $(".droppable-above").removeClass("droppable-above");
        $(".droppable-in").removeClass("droppable-in");
        $(".treeViweHelper").css("opacity", "1").css("top", "").removeClass("treeViweHelper");
    ...
    $("tr:not(.notsortaable)", table).each(function () {
        var $current = $(this);
        var currentebene = parseInt($current.attr("data-tree-ebene"));
        ...
        var elemente = 0;
        while (nextebene > currentebene) {
            if (nextebene == (currentebene + 1)) {
                elemente++;
                $next.attr("data-parent", $current.attr("data-row-id"));
                
            }
           // $next.find(".treeview").eq(currentebene).find(".dcol:first-child").addClass("abbild_halfkreuz");
            $next = $next.next();
            nextebene = parseInt($next.attr("data-tree-ebene"));
        }
        ...
        while (nextebene > currentebene) {
            if (nextebene == (currentebene + 1)) {
                $next.attr("data-parent", $current.attr("data-row-id"));
                elemente_minus=elemente_minus-1;
                if (elemente_minus==0) {
                    $next.find(".treeview").eq(currentebene).find(".dcol:first-child").addClass("abbild_ecke");
                } else {
                    $next.find(".treeview").eq(currentebene).find(".dcol:first-child").addClass("abbild_halfkreuz");
                }
            } else {
                if (elemente_minus>0)
                $next.find(".treeview").eq(currentebene).find(".dcol:first-child").addClass("abbild_linie");
            }
        ...
        $current.find(".treeview_ab").find(".dcol").append("<span class='color-" + currentebene + "' style='margin-right:auto;'>" + elemente + "</span>");
    });
}
...
function tableSortable($that, items) {
...
//console.log((mouseYPosition-jq1112(document).scrollTop())+" "+(SCROLL_AREA_TOP_HEIGHT)+" "+($that.find(".dtable > tbody .notsortaable").offset().top+$that.find(".dtable > tbody .notsortaable").height()));
        ...
        stop: function (event, ui) {
            ...
            $(".treeViweHelper").css("display", "").removeClass(".treeViweHelper");
...
function frozeHead(tag) {
    ...
    if (typeof tag != typeof undefined && tag != "") {
    } else {
        tag = ".table-jsfixed";
    }
...
var templateSignatures=[];


function templateSignatureToForm() {
    for (var i=0; i<templateSignatures.length;i++) {
        var sig=templateSignatures[i];
        var wert=sig.toDataURL();
        sig.element.find(".sig_hidden").val(wert);
    }
}

function templateSignatureToForm_required(element) {
    for (var i=0; i<templateSignatures.length;i++) {
        var sig=templateSignatures[i];
        var wert=sig.toDataURL();
        if (element.attr("data-id")===sig.element.find(".sig_hidden").attr("data-id")) {
            if (sig.element.find(".sig_hidden").val()==wert) {
                return true;
            } else {
                return false;
            }
...
function templateSignaturRequired_($that) {
    var input=$that.find(".sig_hidden");
    var check=true;
    if (templateSignatureToForm_required(input)) {
        $that.addClass("required-invalid");
        check=false;
    } else {
        $that.removeClass("required-invalid");
...
function templateSignaturRequired($that) {
    var input=$that.find("input");
    $that.addClass("required");
}

var global_signature=0;

function templateSignatureInit($that) {
    var wrapper = $that.find(".m-signature-pad")[0],
  ...
  //  signaturePad.backgroundColor = ""; //Hintergrundfarbe
    
    signaturePad.element=$that;
    $that.find(".sig_hidden").attr("data-id",global_signature);
    
    
    templateSignatures.push(signaturePad);
    
    $that.find(".m-signature-pad-clear").unbind("click.sig_clear").bind("click.sig_clear", function() {
        signaturePad.clear();
    });
