...
function twitter_checkdisplay(anzahl) {
    var i=0;
    var speicher=new Array();
    var timestamp="";
    jq1112(".twitter-article").each(function() {
        jq1112(this).css("display","none");
        timestamp=jq1112(this).attr("data-twittertimestamp");
        speicher[i]=timestamp;
        i++;
    });
    speicher.sort(function(a, b){return b-a});
    for (var i=0;i<anzahl;i++) {
        if (speicher[i])
        jq1112("."+"twittertimestamp_"+speicher[i]).css({display:""});
    }
}

function twitterTicker(id, query, anzahl) {
    ...
    var domain = "http://www.prof4mail.de/twitter/";
    //domain = "http://www.sales4net.de/twitter/";

    url = domain+"twitter_result.php?time="
            + now.getTime() + "&q=" + query + "&id=" + id + "&anzahl=" + anzahl;
    var script = document.createElement("script");
    script.setAttribute("src", url);
    script.setAttribute("type", "text/javascript");
    script.setAttribute("class", "script_"+id);
    document.getElementsByTagName("head")[0].appendChild(script);
}

function twitter(feeds, anzahl, id) {
    var displaylimit = anzahl;
    var showdirecttweets = false;
    var showretweets = true;
    var showtweetlinks = true;
    var showtweetactions = true;
    var showretweetindicator = true;
    //alert(feeds['statuses']);
    //	   feeds = feeds['statuses'];
    var feedHTML = '<div class="twitter-article" id="tw0">No results</div>';
    var displayCounter = 1;

    var headerHTML = '';
    headerHTML += '<div id="twitter-header">';/*<a href="https://twitter.com/" target="_blank"><img src="img/twitter_bird_light.png" width="34" style="float:left;padding:3px 12px 0px 6px" alt="twitter bird" /></a>*/
        ...
        for (var i = 0; i < feeds.length; i++) {
            ...
            if (((showretweets == true) || ((isaretweet == false) && (showretweets == false))) && ((showdirecttweets == true) || ((showdirecttweets == false) && (isdirect == false)))) {
                if ((feeds[i].text.length > 1) && (displayCounter <= displaylimit)) {
                    ...
                    feedHTML += '<div class="twitter-article twittertimestamp_'+twitter_gettimestamp(feeds[i].created_at)+'" data-twittertimestamp="'+twitter_gettimestamp(feeds[i].created_at)+'" id="tw' + displayCounter + '">';
    ...
    jq1112(id).html(headerHTML + feedHTML);
    
    
    //Add twitter action animation and rollovers
    if (showtweetactions == true) {
        jq1112('.twitter-article').hover(function () {
            jq1112(this).find('.twitter-actions').css({'visibility': 'visible','display': 'block', 'opacity': 0, 'margin-top': 3});
            jq1112(this).find('.twitter-actions').animate({'opacity': 1, 'margin-top': 3}, 200);
        }, function () {
            jq1112(this).find('.twitter-actions').animate({'opacity': 0, 'margin-top': 3}, 120, function () {
        ...
        jq1112('.twitter-actions a').click(function () {
    ...
    function animatetweets() {
        ...
        for (var i = starttweet; i < displayCounter; i++) {
            jq1112('#tw' + i).css({'display': 'none', 'opacity': 0});
        }
        fadetweet();
        function fadetweet() {

            jq1112('#tw' + animatetweet).css({'display': 'block'});
            jq1112('#tw' + animatetweet).css('margin-top', -fadeoffsetin);
            jq1112('#tw' + animatetweet).animate({'opacity': 1, 'margin-top': 0}, tweetfadetime, function () {
                jq1112('#tw' + animatetweet).delay(tweetdelaytime).animate({'opacity': 0, 'margin-top': fadeoffsetout}, tweetfadetime, function () {
                    jq1112('#tw' + animatetweet).css({'display': 'none', 'margin-top': 0});
...
function twitter_gettimestamp(time_value) {
    var tag=time_value.substr(0,3);
    time_value=time_value.replace(tag,tag+",");
    time_value=time_value.replace("+0000","");

    var date2 = new Date(time_value+"");
    ...
    var timestamp = ("" + date2.getFullYear() + m + d + h + min + s);
...
jq1112(window).load(function ($) {
    ...
    JavascriptLoader.push(function () {
        if (typeof InfoPanel=="object" && document == topmenu.document && global_infopanel) {
            InfoPanel.create();
        }
    });
